...
std::array<std::uint8_t, 256> initialize_inverted_sbox(){
...
void mix_column(std::uint8_t *state) noexcept{
	std::uint8_t col[4];
	col[0] = state[0];
	col[1] = state[1];
	col[2] = state[2];
	...
	state[0] = gf_mul[col[0]][0] ^ gf_mul[col[1]][1] ^ col[2] ^ col[3];
	state[1] = col[0] ^ gf_mul[col[1]][0] ^ gf_mul[col[2]][1] ^ col[3];
	state[2] = col[0] ^ col[1] ^ gf_mul[col[2]][0] ^ gf_mul[col[3]][1];
	state[3] = gf_mul[col[0]][1] ^ col[1] ^ col[2] ^ gf_mul[col[3]][0];
}

void invert_mix_column(std::uint8_t *state) noexcept{
	auto a = gf_mul[state[0]];
	auto b = gf_mul[state[1]];
	auto c = gf_mul[state[2]];
	auto d = gf_mul[state[3]];

	state[0] = a[5] ^ b[3] ^ c[4] ^ d[2];
	state[1] = a[2] ^ b[5] ^ c[3] ^ d[4];
	state[2] = a[4] ^ b[2] ^ c[5] ^ d[3];
...
std::uint32_t subword(uint32_t word){
	...
	result = (std::uint32_t)sbox[word & 0xFF];
	result |= (std::uint32_t)sbox[(word >> 8) & 0xFF] << 8;
	result |= (std::uint32_t)sbox[(word >> 16) & 0xFF] << 16;
	result |= (std::uint32_t)sbox[(word >> 24) & 0xFF] << 24;
